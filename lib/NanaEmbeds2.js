  ...
  async getInfoEmbed(id, msg) {
    ...
    if (banned)
      return msg.channel
        .send(
<<<<<<< HEAD
          `:warning: Sorry, I cannot display the doujin because its contain **BANNED** tag\n\nNhentai Doujin Code: ${id}`
=======
...
>>>>>>> 0cfad0064616493d8536f4c08cabb867b3b71961
    ...
    let m = await msg.author.send(embed);
    let mic = msg.reply(' Check DM from me for the doujin my horny mate....')
    this.getEmoji(id, m, msg, mic);
  }
  ...
  async getEmoji(id, m, msg) {
    ...
    forwards.on("collect", async f => {
      ...
      let r = await msg.author.send(read);
  ...
  async download(res, type) {
    ...
    const embed = new MessageEmbed()
      ...
      .setTimestamp()
      .setDescription(
        `To start download, click the doujin title above.\n\nFor help DM B.S#2827`
      );
    return embed;
  }
}
module.exports = NanaEmbeds2;
